...
import { getPurchaseReturnById, updatePurchaseReturn } from '@/db/purchase-return';
...
export default function EditPurchaseReturnScreen() {
  ...
  const handleSave = async () => {
    ...
    try {
      setSaving(true);
      await updatePurchaseReturn(returnData.id, user.id, {
  ...
  const handleAddItem = () => {
    ...
    const newItem = {
      ...
      total: selectedProduct.purchasePrice,
      notes: '',
    };
    setReturnData({
      ...returnData,
      items: [...(returnData.items || []), newItem],
      subtotal: (returnData.subtotal || 0) + selectedProduct.purchasePrice,
      total: (returnData.total || 0) + selectedProduct.purchasePrice,
    });
            ...
            <ScrollView>
              {products.map((product) => (
                ...
                >
                  ...
                  <Text style={styles.modalItemSubtext}>${(product.purchasePrice / 100).toFixed(2)}</Text>
                </TouchableOpacity>
              ))}
            </ScrollView>
          </View>
