...
export default function EditVendorScreen() {
  ...
  const loadVendorData = async () => {
    if (!id) {
      setSnackbarMessage("Vendor ID is required");
    ...
    try {
      setIsLoading(true);
      const vendor = await dbVendor.getVendorById(Number(id));
      ...
      if (vendor) {
        setFormData({
          ...
          tags: vendor.tags ? vendor.tags.split(',').map(t => t.trim()).filter(Boolean) : [],
          notes: vendor.notes || "",
        });
      } else {
        setSnackbarMessage("Vendor not found");
        setSnackbarVisible(true);
      }
    } catch (error) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Update in database
      const success = await dbVendor.updateVendor(Number(id), updatedVendor);
      
      if (success) {
        setSnackbarMessage("Vendor updated successfully");
        setSnackbarVisible(true);
        
        // Navigate back to vendor details after a short delay
        setTimeout(() => {
