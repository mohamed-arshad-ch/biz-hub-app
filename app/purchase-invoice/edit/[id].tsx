...
import { getPurchaseInvoiceById, updatePurchaseInvoice } from '@/db/purchase-invoice';
...
export default function EditPurchaseInvoiceScreen() {
  ...
  const [formData, setFormData] = useState<PurchaseInvoiceFormData | null>(null);
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        setFormData({
          ...
          tax: (inv.tax ?? 0) / 100,
          total: inv.total / 100,
          status: (inv.status as PurchaseInvoiceFormData['status']) ?? 'unpaid',
          notes: inv.notes ?? '',
        });
  ...
  const selectStatus = (status: string) => {
    if (!formData) return;
    setFormData({ ...formData, status: status as PurchaseInvoiceFormData['status'] });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await updatePurchaseInvoice(Number(id), user.id, {
            ...
            >
              <View style={styles.selectButtonContent}>
                ...
                <Text style={styles.selectButtonText}>
                  {vendors.find(v => v.id.toString() === formData.vendorId)?.name || 'Select Vendor'}
                </Text>
              </View>
          ...
          <View style={styles.modalContainer}>
            <View style={styles.modalContent}>
              <View style={styles.modalHeader}>
                ...
                <TouchableOpacity onPress={() => setShowVendorSheet(false)}>
              ...
              <FlatList
                ...
                renderItem={({ item }) => (
                  ...
                  >
                    ...
                    {item.id.toString() === formData.vendorId && (
