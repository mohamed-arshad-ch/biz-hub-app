...
import { getSalesInvoiceById, deleteSalesInvoice } from '@/db/sales-invoice';
...
export default function SalesInvoiceDetailsScreen() {
  ...
  const fetchInvoice = async () => {
    ...
    try {
      ...
      setInvoice(inv);
      if (inv?.customerId) {
        const cust = await dbCustomer.getCustomerById(inv.customerId);
        setCustomer(cust);
      }
    } catch (error) {
  ...
  const handleDelete = async () => {
    ...
    Alert.alert('Delete Invoice', 'Are you sure you want to delete this invoice?', [
      ...
      { text: 'Delete', style: 'destructive', onPress: async () => {
        try {
          await deleteSalesInvoice(Number(id), user.id);
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerActions}>
          <TouchableOpacity 
            ...
            onPress={() => router.push(`/sales-invoice/edit/${id}`)}
          >
