...
import { getSaleById } from "@/mocks/salesData";
...
export default function EditSaleScreen() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      new Promise(resolve => {
        setTimeout(() => {
          setCustomers(getCustomersData(20));
          resolve(null);
        }, 500);
      }),
      // Load products
      new Promise(resolve => {
        setTimeout(() => {
          setProducts(getProductsData(30));
          resolve(null);
        }, 500);
      }),
      // Load sale data
        ...
        setTimeout(() => {
          if (id) {
            ...
            if (saleData) {
              ...
              if (saleData.dueDate) {
                setDueDate(new Date(saleData.dueDate));
              }
              // Find and set customer
              const customer = getCustomersData(20).find(c => c.name === saleData.customer);
              if (customer) {
              ...
              // Mock items since our mock data doesn't include them
              ...
              if (saleData.status === "paid") {
                setAmountPaid(saleData.amount.toString());
              }
            }
          }
          resolve(null);
