...
import { createSalesReturn } from '@/db/sales-return';
import * as dbCustomer from '@/db/customer';
import * as dbProduct from '@/db/product';
import * as dbSalesInvoice from '@/db/sales-invoice';
...
export default function NewSalesReturnScreen() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        setLoading(true);
        const dbInvoices = await dbSalesInvoice.getSalesInvoices(user.id);
  ...
  const selectStatus = (status: string) => {
    setFormData({ ...formData, status: status as SalesReturnFormData['status'] });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      router.replace('/sales-return/sales-return');
    } catch (error) {
      console.error('Error creating sales return:', error);
      Alert.alert('Error', 'Failed to create sales return.');
    }
  };
        ...
        <View style={styles.section}>
            ...
            >
              ...
              <Text 
                style={[
                  ...
                  !formData.invoiceId && styles.placeholderText
                ]}
              >
          ...
          <View style={styles.row}>
            <View style={[styles.formGroup, { flex: 1, marginRight: 8 }]}> 
              <Text style={styles.label}>Return # <Text style={styles.required}>*</Text></Text>
          ...
          <View style={styles.sheetContainer}>
            ...
            <FlatList
              ...
              renderItem={({ item }) => (
                ...
                >
                  <Text style={styles.sheetItemText}>#{item.invoiceNumber}</Text>
                  {formData.invoiceId === item.id.toString() && (
              ...
              renderItem={({ item }) => (
                ...
                >
                  <View style={{flex: 1}}>
                    ...
                    <Text style={styles.sheetItemPrice}>${(item.sellingPrice / 100).toFixed(2)}</Text>
                  </View>
