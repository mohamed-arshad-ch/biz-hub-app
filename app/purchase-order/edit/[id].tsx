...
export default function EditPurchaseOrderScreen() {
  ...
  const loadPurchaseOrder = async () => {
    ...
    try {
      ...
      setOrder({
        ...orderResult,
        vendorName: orderResult.vendorName || 'Unknown Vendor',
        status: orderResult.status || 'pending',
        tax: orderResult.tax || 0,
        discount: orderResult.discount || 0,
        notes: orderResult.notes || '',
        shippingAddress: orderResult.shippingAddress || '',
        billingAddress: orderResult.billingAddress || '',
        paymentTerms: orderResult.paymentTerms || '',
        dueDate: orderResult.dueDate || new Date().toISOString(),
      ...
      // Set form values
      setSelectedVendor({ id: orderResult.vendorId, name: orderResult.vendorName || 'Unknown Vendor' });
      setOrderDate(new Date(orderResult.orderDate));
      setDueDate(orderResult.dueDate ? new Date(orderResult.dueDate) : null);
      setItems(itemsResult.map(item => ({
        ...item,
        productName: item.productName || 'Unknown Product',
        discount: item.discount || 0,
        tax: item.tax || 0
      ...
      setShippingAddress(orderResult.shippingAddress || '');
      setBillingAddress(orderResult.billingAddress || '');
      setPaymentTerms(orderResult.paymentTerms || '');
    } catch (error) {
